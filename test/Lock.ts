...
describe("Lock", function () {
    ...
    // We use networkHelpers.loadFixture to run this setup once, snapshot that state,
    ...
    describe("Deployment", function () {
        ...
        it("Should receive and store the funds to lock", async function () {
            const {lock, lockedAmount} = await networkHelpers.loadFixture(deployOneYearLockFixture,);
    ...
    describe("Withdrawals", function () {
        describe("Validations", function () {
            ...
            it("Shouldn't fail if the unlockTime has arrived and the owner calls it", async function () {
                ...
                await expect(lock.withdraw()).not.to.be.revert(ethers);
            });
        });
        ...
        describe("Transfers", function () {
            it("Should transfer the funds to the owner", async function () {
                ...
                await expect(lock.withdraw()).to.changeEtherBalances(ethers, [owner, lock], [lockedAmount, -lockedAmount],);
            });
        });
    });
    ...
    describe("Message", function () {
        ...
        it("Should set the right message", async function () {
            const {lock} = await networkHelpers.loadFixture(deployOneYearLockFixture,);
            await expect(lock.setMessage("test")).to.emit(lock, "SetMessage").withArgs("test");
            expect(await lock.message()).to.equal("test");
        });
    });
});
